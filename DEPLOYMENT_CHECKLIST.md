...
3. Run `supabase/security-fixes.sql` to enforce `security_invoker` on sensitive views (e.g., `public.farmer_profit`).
4. Ensure JWT includes `app_metadata.farm_id` for multi-tenant isolation.
5. Ensure application flow always supplies an explicit `farm_id` on writes.
