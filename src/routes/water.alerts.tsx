...
function Page() {
  ...
  const merged = useMemo(() => {
    ...
    return [...iot, ...enterpriseAlerts].sort(
  ...
  return (
    ...
    >
      <div className="mb-4 grid gap-4 md:grid-cols-4">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-sm">IoT Alerts</CardTitle>
          </CardHeader>
          <CardContent className="text-2xl font-semibold">{waterAlerts.length}</CardContent>
        </Card>
        <Card>
          <CardHeader>
              ...
              return (
                <div key={alert.id} className="rounded-lg border p-3">
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    Scope: {alert.scope ?? "global"} · {new Date(alert.createdAt).toLocaleString()}{" "}
                    · Source: {alert.source}
                  </p>
                </div>
              );
