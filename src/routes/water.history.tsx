...
function Chart({ title, k, color, unit }: { title: string; k: string; color: string; unit: string }) {
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between pb-2"><CardTitle className="text-base">{title}</CardTitle><span className="text-xs text-muted-foreground">Last 24h ({unit})</span></CardHeader>
      <CardContent className="h-56">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data}>
            <defs><linearGradient id={`g-${k}`} x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={color} stopOpacity={0.4} /><stop offset="100%" stopColor={color} stopOpacity={0} /></linearGradient></defs>
            ...
            <Area type="monotone" dataKey={k} stroke={color} fill={`url(#g-${k})`} strokeWidth={2} />
          </AreaChart>
        </ResponsiveContainer>
      </CardContent>
    </Card>
