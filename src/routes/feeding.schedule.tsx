...
const FEEDING_SCHEDULE_KEY = "aquasmart_feeding_schedule";

function loadSchedule(): FeedScheduleRow[] {
  if (typeof window === "undefined") return [];
  const raw = localStorage.getItem(FEEDING_SCHEDULE_KEY);
  if (!raw) return [];
  try {
    const parsed = JSON.parse(raw) as FeedScheduleRow[];
...
function saveSchedule(rows: FeedScheduleRow[]) {
  if (typeof window === "undefined") return;
  localStorage.setItem(FEEDING_SCHEDULE_KEY, JSON.stringify(rows));
}
...
function Page() {
  const [rows, setRows] = useState<FeedScheduleRow[]>(() => loadSchedule());
              ...
              {rows.map((r) => (
                <li key={r.id} className="flex items-center justify-between rounded-md border p-3 text-sm">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <Button variant="outline" size="sm" onClick={() => removeSchedule(r.id)}>Remove</Button>
                  </div>
                </li>
              ))}
            </ul>
