...
export async function buildFinancialStatement(range?: DateRange): Promise<FinancialStatement> {
  ...
  const incomeJournalTransactions: JournalTransaction[] = incomeRows.map(incomeToJournal);
  const expenseJournalTransactions: JournalTransaction[] = expenseRows.map(expenseToJournal);
  const journalTransactions: JournalTransaction[] = [
    ...incomeJournalTransactions,
    ...expenseJournalTransactions,
