...
-- Access expression:
-- coalesce(auth.jwt() -> 'app_metadata' ->> 'farm_id', '')

create policy if not exists "finance_income_select" on finance_income
for select using (farm_id = coalesce(auth.jwt() -> 'app_metadata' ->> 'farm_id', ''));
create policy if not exists "finance_income_insert" on finance_income
for insert with check (farm_id = coalesce(auth.jwt() -> 'app_metadata' ->> 'farm_id', ''));
create policy if not exists "finance_income_update" on finance_income
for update using (farm_id = coalesce(auth.jwt() -> 'app_metadata' ->> 'farm_id', ''))
with check (farm_id = coalesce(auth.jwt() -> 'app_metadata' ->> 'farm_id', ''));
create policy if not exists "finance_income_delete" on finance_income
for delete using (farm_id = coalesce(auth.jwt() -> 'app_metadata' ->> 'farm_id', ''));
